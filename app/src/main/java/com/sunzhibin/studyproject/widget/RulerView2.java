...
public class RulerView2 extends View {
    ...
    private float mLineMaxHeight = 20;
    private float mLineMidHeight = 15;
    private float mLineMinHeight = 10;
    private int mLineColor = 1;

    private int mCurLineColor = 0xffff0000;
    private float mLineCurHeight = 23;
    private float mCurLineWidth = 2;
    ...
    protected void init(Context context, AttributeSet attrs) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.RulerView2);

        mAlphaEnable = typedArray.getBoolean(R.styleable.RulerView2_alphaEnable, mAlphaEnable);

        mLineSpaceWidth = typedArray.getDimension(R.styleable.RulerView2_lineSpaceWidth, dp2px(context, mLineSpaceWidth));
        mLineWidth = typedArray.getDimension(R.styleable.RulerView2_lineWidth, dp2px(context, mLineWidth));
        mCurLineWidth = typedArray.getDimension(R.styleable.RulerView2_lineCurWidth, dp2px(context, mCurLineWidth));
        mLineMaxHeight = typedArray.getDimension(R.styleable.RulerView2_lineMaxHeight, dp2px(context, mLineMaxHeight));
        mLineMidHeight = typedArray.getDimension(R.styleable.RulerView2_lineMidHeight, dp2px(context, mLineMidHeight));
        mLineMinHeight = typedArray.getDimension(R.styleable.RulerView2_lineMinHeight, dp2px(context, mLineMinHeight));
        mLineCurHeight = typedArray.getDimension(R.styleable.RulerView2_lineCurHeight, dp2px(context, mLineCurHeight));

        mLineColor = typedArray.getColor(R.styleable.RulerView2_lineColor, mLineColor);
        mCurLineColor = typedArray.getColor(R.styleable.RulerView2_lineCurColor, mCurLineColor);

        mTextSize = typedArray.getDimension(R.styleable.RulerView2_textSize, dp2px(context, mTextSize));
        mTextColor = typedArray.getColor(R.styleable.RulerView2_textColor, mTextColor);
        mTextMarginTop = typedArray.getDimension(R.styleable.RulerView2_textMarginTop, dp2px(context, mTextMarginTop));

        mSelectorValue = typedArray.getFloat(R.styleable.RulerView2_selectorValue, 0.0f);
        mMinValue = typedArray.getFloat(R.styleable.RulerView2_minValue, 0.0f);
        mMaxValue = typedArray.getFloat(R.styleable.RulerView2_maxValue, 100.0f);
        mPerValue = typedArray.getFloat(R.styleable.RulerView2_perValue, 0.1f);
        ...
        mCurLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mCurLinePaint.setStrokeWidth(mCurLineWidth);
        mCurLinePaint.setDither(true);
        mCurLinePaint.setAntiAlias(true);
        mCurLinePaint.setColor(mCurLineColor);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mTotalLine; i++) {
            ...
            canvas.drawLine(mWidth / 2, 0, mWidth / 2, mLineCurHeight, mCurLinePaint);
        }
    }
